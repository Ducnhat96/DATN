...
const BankList: ComponentType<IProps> = (props: IProps) => {
  ...
  const triggerPayment = () => {
    if (bankId !== 0) {
      ...
      redirectToBaoKim(lists!.uuid, bankId)
        .then(res => {
          const url = res.data;
        ...
        <Grid container spacing={2}>
          <Grid item md={12} xs={12}>
            ...
            <Typography variant="subtitle2">
              Bạn vui lòng chọn một trong các hình thức thanh toán bên dưới để
              tiến thành thanh toán. Nếu bạn thanh toán qua thẻ ATM nội địa, vui
              lòng chọn ngân hàng phù hợp. Việc thanh toán sẽ được tiến hành
              thông qua cổng thanh toán điện tử{" "}
              <a
                style={{
                  fontWeight: 700,
                  cursor: "pointer",
                ...
                <Grid container spacing={1}>
                  ...
                  <Grid item md={12} xs={12}>
                    <i className={classes.caption}>
                      Là hình thức thanh toán trực tuyến an toàn và bảo mật qua
                      các thẻ nội địa do các ngân hàng phát hành (cổng thanh
                      toán Bảo Kim). Dịch vụ được kích hoạt tự động ngay sau khi
                      thanh toán, đơn giản và thuận tiện hơn cho khách hàng.
                    </i>
                  </Grid>
                  <Grid item md={12} xs={12}>
                    <Typography variant="subtitle2">
                  ...
                  <Grid item md={12} xs={12}>
                    <i className={classes.caption}>
                      - Để thanh toán bằng thẻ ATM, thẻ của Quý khách phải được
                      đăng ký và kích hoạt chức năng thanh toán trực tuyến với
                      ngân hàng (internet banking) trước khi sử dụng.
                    </i>
                    <br />
                    <i className={classes.caption}>
                      - Các thông tin thẻ tín dụng của Quý khách sẽ được bảo mật
                      và được xác nhận với ngân hàng phát hành thẻ.
                    </i>
                  </Grid>
                  <Grid item md={12} xs={12}>
                        ...
                        _.map(listBank, bank => (
                          ...
                          >
                            <img
                              src={bank.logo_url}
                              alt={bank.name}
                              className={classNames(classes.bankImg, {
                                [classes.bankImgFocus]:
                                  bank.id === bankId.toString()
                              })}
                            />
          ...
          <Grid item md={12} xs={12}>
            ...
            >
              {paymentPending
                ? "Bạn đang được chuyển sang Bảo Kim để tiếp tục thanh toán"
                : "Thanh toán"}
            </Button>
          </Grid>
        </Grid>
