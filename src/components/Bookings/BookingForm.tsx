...
const FormValidationSchema = Yup.object().shape({
  ...
  guestName: Yup.string().when(
    "isSomeOneElse",
    (status: boolean, schema: StringSchema) => {
      return status
        ? schema
            .required("Vui lòng điền tên")
            .min(2, "Phải có ít nhất 2 ký tự")
...
const BookingForm: ComponentType<IProps> = (props: ILocalProps) => {
  ...
  useEffect(() => {
    if (isSubmitting && !isValidating) {
      errors.firstName
        ? scrollDefault("firstName")
        : errors.lastName
        ? scrollDefault("lastName")
        : errors.email
        ? scrollDefault("email-lists")
        : errors.phone
        ? scrollDefault("phone-number")
        : errors.country
        ? scrollDefault("country")
  ...
  return (
    <Fragment>
      {room && room!.settings !== null ? (
        room!.settings.no_booking_cancel == 0 ? (
        ...
        <Paper square className={classes.paperCustomHot}>
          <List className={classes.root}>
            <ListItem>
              ...
              <ListItemText
                primary={
                  <Typography variant="h6" style={{ color: Orange[600] }}>
                    {room!.hot_txt} Hãy hoàn thành đặt chỗ sớm
                  </Typography>
                }
              />
            </ListItem>
            ...
            <Grid container spacing={2}>
              <Grid item xs={12}>
                ...
                <Typography variant="body2" style={{ color: "#b9b8b8" }}>
                  Tên của khách phải khớp với tên ghi trên giấy tờ tuỳ thân dùng
                  để nhận phòng.
                </Typography>
              </Grid>
          ...
          <Grid container justify="flex-start">
            <Grid item>
              <Typography variant="body2" style={{ padding: "15px" }}>
                Khi gửi yêu cầu đặt chỗ này, quý khách xác nhận rằng quý khách
...
const FormMilk = withFormik({
  ...
  handleSubmit: (
    ...
    bags: FormikBag<ILocalProps, IFormikValues>
  ) => {
