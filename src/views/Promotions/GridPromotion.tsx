...
const useStyles = makeStyles<Theme, IPromotions>((theme: Theme) =>
  createStyles({
    card: {
      // maxWidth: 400,
      display: "block",
      ...
      <div className={"flip-card flip-card-vertical"}>
        ...
        <div className={"front-card"}>
          ...
          <Card
            ...
            raised={false}
            style={{
              backgroundImage: `url(${`https://s3-ap-southeast-1.amazonaws.com/westay-img/sm/${promotion.image}`})`
            }}
          >
          ...
          <Card elevation={4} raised={false}>
            <CardContent className={classes.cardContentHidden}>
              ...
              <Typography component="p" className={classes.message}>
                Mã giảm giá được áp dụng từ ngày{" "}
                {moment(promotion.date_start).format("LL")} đến ngày{" "}
                {moment(promotion.date_end).format("LL")}
              </Typography>
              <div className={classes.line}></div>
              {promotion.coupons.data[0].settings.bind.length == 0 ? (
                <Typography component="p" className={classes.message}>
                  Mã giảm giá giảm tối đa{" "}
                  {formatMoney(promotion.coupons.data[0].max_discount)} cho đặt
                  phòng tối thiểu từ{" "}
                  {formatMoney(promotion.coupons.data[0].settings.min_price)}{" "}
                  đồng. Chi tiết liên hệ : 0946 746 417
                </Typography>
              ) : (
                _.map(promotion.coupons.data[0].settings.bind, criteria => (
                  <ul>
                    {criteria === "rooms" ? (
                      <li>
                        Mã giảm giá chỉ áp dụng cho một số phòng nhất định
                      </li>
                    ) : (
                      ""
                    )}
                    {criteria === "cities" ? (
                      <li>
                        Mã giảm giá chỉ áp dụng cho đặt phòng tại một số tỉnh,
                        thành phố
                      </li>
                    ) : (
                      ""
                    ...
                    {criteria === "districts" ? (
                      <li>Mã giảm giá chỉ áp dụng cho một số quận, huyện</li>
                    ) : (
                      ""
                    )}
                    {criteria === "days" ? (
                      <li>Mã giảm giá chỉ áp dụng cho một số ngày nhất định</li>
                    ) : (
                      ""
                    )}
                    {criteria === "booking_type" &&
                    promotion.coupons.data[0].settings.booking_type !== 3 ? (
                      <li>
                        Mã giảm giá chỉ áp dụng cho loại đặt phòng theo{" "}
                        {promotion.coupons.data[0].settings.booking_type == 1
                          ? "Giờ"
                          : "Ngày"}
                      </li>
                    ) : (
                    ...
                    {criteria === "booking_create" ? (
                      <li>
                        Mã giảm giá chỉ áp dụng cho đặt phòng được tạo trong
                        khoảng ngày từ{" "}
                        {moment(
                          promotion.coupons.data[0].settings.booking_create[0]
                        ).format("LL")}{" "}
                        tới{" "}
                        {moment(
                          promotion.coupons.data[0].settings.booking_create[1]
                        ).format("LL")}{" "}
                      </li>
                    ) : (
                      ""
                    ...
                    {criteria === "booking_create" ? (
                      <li>
                        Mã giảm giá chỉ áp dụng cho đặt phòng có thời gian
                        checkin - checkout từ{" "}
                        {moment(
                          promotion.coupons.data[0].settings.booking_stay[0]
                        ).format("LL")}{" "}
                        tới{" "}
                        {moment(
                          promotion.coupons.data[0].settings.booking_stay[1]
                        ).format("LL")}{" "}
                      </li>
                    ) : (
                      ""
